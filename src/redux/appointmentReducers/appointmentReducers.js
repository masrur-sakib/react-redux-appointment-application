import {
  CHANGE_BOOKED_SLOT_COLOR,
  INSERT_SUBMITTED_DATA,
  SAVE_APPOINTMENT,
} from "../appointmentActions/appointmentActions";
...
const appointmentReducers = (state = initialState, action) => {
  switch (action.type) {
    ...
    case INSERT_SUBMITTED_DATA:
      ...
      if (appointmentData) {
        document.getElementById("firstName" + action.formId).value =
          appointmentData.firstName;
        document.getElementById("lastName" + action.formId).value =
          appointmentData.lastName;
        document.getElementById("phone" + action.formId).value =
